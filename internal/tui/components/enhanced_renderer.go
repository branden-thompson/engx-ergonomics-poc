package components

import (
	"fmt"
	"strings"
	"time"
	progresssim "github.com/bthompso/engx-ergonomics-poc/internal/simulation/progress"
)

// ANSI color codes for styling
const (
	// Basic colors
	colorReset         = "\033[0m"
	colorWhite         = "\033[97m"  // Bright white
	colorBlue          = "\033[94m"  // Blue for running/installing
	colorYellow        = "\033[93m"  // Yellow for stalling/paused/JavaScript
	colorRed           = "\033[91m"  // Red for failed
	colorGreen         = "\033[92m"  // Green for done/installed
	colorLightGrey     = "\033[90m"  // Darker grey for lines
	colorGrey          = "\033[37m"  // Light grey for skipped items

	// Header-specific colors
	colorBrightMagenta = "\033[95m"  // Bright magenta for app name/paths
	colorBrightOrange  = "\033[38;5;208m"  // Bright orange for PRODUCTION READY
	colorMagenta       = "\033[35m"  // Magenta for Objective-C

	// Text styles
	styleBold   = "\033[1m"
	styleItalic = "\033[3m"
	styleReset  = "\033[22m"
)

// Helper function to create colored separator lines
func (r *EnhancedRenderer) renderSeparatorLine() string {
	return fmt.Sprintf("%s%s%s", colorLightGrey, strings.Repeat("-", r.totalWidth), colorReset)
}

// EnhancedRenderer renders progress in the comprehensive template format
type EnhancedRenderer struct {
	steps         []Step
	currentStep   int
	appName       string
	targetDir     string
	template      string
	archetype     progresssim.ArchetypeType
	startTime     time.Time
	isDevOnly     bool

	// Layout configuration
	stepNameWidth    int
	progressBarWidth int
	totalWidth       int

	// Component sections (all 6 sections from templates details)
	coreTechnologies     []Component
	engxIntegrations     []Component
	qualityTesting       []Component
	userAnalyticsTesting []Component
	agenticCapabilities []Component
	deploymentCapabilities []Component

	// Component management
	componentManager *ComponentManager
}

// Component represents any technology component with status
type Component struct {
	Name   string
	Status string // "installed", "installing", "queued"
	Icon   string // "[✓]", "[✓ ]", "[ ]"
}


// ComponentInstallationState represents different installation states
type ComponentInstallationState int

const (
	ComponentQueued ComponentInstallationState = iota
	ComponentInstalling
	ComponentSkipped
	ComponentInstalled
	ComponentFailed
)

// NewEnhancedRenderer creates a new enhanced renderer with comprehensive layout
func NewEnhancedRenderer(appName, targetDir, template string, stepNames []string, isDevOnly bool) *EnhancedRenderer {
	steps := make([]Step, len(stepNames))
	for i, name := range stepNames {
		steps[i] = Step{
			Name:     name,
			Status:   StepPending,
			Progress: 0.0,
			SubSteps: make([]string, 0),
		}
	}

	// Calculate optimal layout dimensions
	maxStepNameLength := 0
	for _, name := range stepNames {
		if len(name) > maxStepNameLength {
			maxStepNameLength = len(name)
		}
	}

	// Initialize component sections for prod-web archetype (all start as queued)
	coreTechnologies, engxIntegrations, qualityTesting, userAnalyticsTesting, agenticCapabilities, deploymentCapabilities := getArchetypeComponents(progresssim.ArchetypeReactProd, isDevOnly)

	return &EnhancedRenderer{
		steps:             steps,
		currentStep:       0,
		appName:           appName,
		targetDir:         targetDir,
		template:          template,
		startTime:         time.Now(),
		isDevOnly:         isDevOnly,
		stepNameWidth:     42, // Fixed width for alignment
		progressBarWidth:  30, // Fixed width like in template
		totalWidth:        89, // Match template width
		coreTechnologies:       coreTechnologies,
		engxIntegrations:       engxIntegrations,
		qualityTesting:         qualityTesting,
		userAnalyticsTesting:   userAnalyticsTesting,
		agenticCapabilities:    agenticCapabilities,
		deploymentCapabilities: deploymentCapabilities,
		componentManager:  NewComponentManagerForArchetype(coreTechnologies, engxIntegrations, qualityTesting, userAnalyticsTesting, agenticCapabilities, deploymentCapabilities),
	}
}

// NewEnhancedRendererWithArchetype creates a new enhanced renderer with archetype-specific components
func NewEnhancedRendererWithArchetype(appName, targetDir, template string, stepNames []string, isDevOnly bool, archetypeType progresssim.ArchetypeType) *EnhancedRenderer {
	steps := make([]Step, len(stepNames))
	for i, name := range stepNames {
		steps[i] = Step{
			Name:     name,
			Status:   StepPending,
			Progress: 0.0,
			SubSteps: make([]string, 0),
		}
	}

	// Calculate optimal layout dimensions
	maxStepNameLength := 0
	for _, name := range stepNames {
		if len(name) > maxStepNameLength {
			maxStepNameLength = len(name)
		}
	}

	// Generate archetype-specific component sections
	coreTechnologies, engxIntegrations, qualityTesting, userAnalyticsTesting, agenticCapabilities, deploymentCapabilities := getArchetypeComponents(archetypeType, isDevOnly)

	return &EnhancedRenderer{
		steps:             steps,
		currentStep:       0,
		appName:           appName,
		targetDir:         targetDir,
		template:          template,
		startTime:         time.Now(),
		isDevOnly:         isDevOnly,
		stepNameWidth:     42, // Fixed width for alignment
		progressBarWidth:  30, // Fixed width like in template
		totalWidth:        89, // Match template width
		coreTechnologies:       coreTechnologies,
		engxIntegrations:       engxIntegrations,
		qualityTesting:         qualityTesting,
		userAnalyticsTesting:   userAnalyticsTesting,
		agenticCapabilities:    agenticCapabilities,
		deploymentCapabilities: deploymentCapabilities,
		componentManager:  NewComponentManagerForArchetype(coreTechnologies, engxIntegrations, qualityTesting, userAnalyticsTesting, agenticCapabilities, deploymentCapabilities),
	}
}

// getArchetypeComponents returns all 6 archetype-specific component sections matching templates details
func getArchetypeComponents(archetypeType progresssim.ArchetypeType, isDevOnly bool) ([]Component, []Component, []Component, []Component, []Component, []Component) {
	var coreTechnologies []Component
	var engxIntegrations []Component
	var qualityTesting []Component
	var userAnalyticsTesting []Component
	var agenticCapabilities []Component
	var deploymentCapabilities []Component

	switch archetypeType {
	case progresssim.ArchetypeReactProd, progresssim.ArchetypeReactDev, progresssim.ArchetypeHackday:
		// React-based applications - matching templates details exactly
		coreTechnologies = []Component{
			{"TypeScript", "queued", ""},
			{"React Router 7", "queued", ""},
			{"Tailwind CSS", "queued", ""},
			{"Radix UI", "queued", ""},
			{"ShadCN-based UI Design System (SUDS)", "queued", ""},
		}
		engxIntegrations = []Component{
			{"TrustBridge SSO", "queued", ""},
			{"gRPC Web", "queued", ""},
			{"CREWS API", "queued", ""},
			{"CATALOG API", "queued", ""},
			{"GRID/HDFS Access", "queued", ""},
			{"GitHub Actions", "queued", ""},
		}
		qualityTesting = []Component{
			{"ESLint + Prettier", "queued", ""},
			{"Vitest Testing", "queued", ""},
			{"Storybook", "queued", ""},
			{"GitHub Pages", "queued", ""},
		}
		userAnalyticsTesting = []Component{
			{"Microsoft Clarity", "queued", ""},
			{"LIX Integration", "queued", ""},
			{"Observe (Service Monitoring)", "queued", ""},
		}
		agenticCapabilities = []Component{
			{"MAE (Auto-Migration)", "queued", ""},
			{"SUDS MCP", "queued", ""},
			{"EngX Agent", "queued", ""},
			{"Co-Pilot", "queued", ""},
		}
		deploymentCapabilities = []Component{
			{"LCD (LinkedIn Continuous Deployment)", "queued", ""},
			{"Docker Ready", "queued", ""},
			{"Azure Cloud", "queued", ""},
		}

	case progresssim.ArchetypeEngXCmd:
		// EngX Command Plugin - matching templates details exactly
		coreTechnologies = []Component{
			{"Terminal UI", "queued", ""},
			{"Interactive Forms", "queued", ""},
			{"Command Structure", "queued", ""},
			{"Plugin System", "queued", ""},
		}
		// No EngX Integrations section in engx-cmd template
		engxIntegrations = []Component{}
		// No Quality & Testing section in engx-cmd template
		qualityTesting = []Component{}
		// No User Analytics & Testing section in engx-cmd template
		userAnalyticsTesting = []Component{}
		agenticCapabilities = []Component{
			{"MAE (Auto-Migration)", "queued", ""},
			{"EngX Agent", "queued", ""},
			{"Co-Pilot", "queued", ""},
		}
		// No Deployment Capabilities section in engx-cmd template
		deploymentCapabilities = []Component{}

	case progresssim.ArchetypeCLI:
		// Command Line Tool - matching templates details exactly
		coreTechnologies = []Component{
			{"Command Structure", "queued", ""},
			{"Flag Parsing", "queued", ""},
			{"Config Files", "queued", ""},
		}
		// No EngX Integrations section in CLI template
		engxIntegrations = []Component{}
		// No Quality & Testing section in CLI template
		qualityTesting = []Component{}
		// No User Analytics & Testing section in CLI template
		userAnalyticsTesting = []Component{}
		agenticCapabilities = []Component{
			{"MAE (Auto-Migration)", "queued", ""},
			{"EngX Agent", "queued", ""},
			{"Co-Pilot", "queued", ""},
		}
		// No Deployment Capabilities section in CLI template
		deploymentCapabilities = []Component{}

	case progresssim.ArchetypeService:
		// Backend Service - matching templates details exactly
		coreTechnologies = []Component{
			{"HTTP API", "queued", ""},
			{"Background Processing", "queued", ""},
			{"Database Integration", "queued", ""},
		}
		// No EngX Integrations section in service template
		engxIntegrations = []Component{}
		// No Quality & Testing section in service template
		qualityTesting = []Component{}
		// No User Analytics & Testing section in service template
		userAnalyticsTesting = []Component{}
		agenticCapabilities = []Component{
			{"MAE (Auto-Migration)", "queued", ""},
			{"EngX Agent", "queued", ""},
			{"Co-Pilot", "queued", ""},
		}
		deploymentCapabilities = []Component{
			{"Docker Support", "queued", ""},
		}

	default:
		// Default/fallback components
		coreTechnologies = []Component{
			{"Core Language", "queued", ""},
			{"Framework", "queued", ""},
			{"Configuration", "queued", ""},
			{"Dependencies", "queued", ""},
		}
		engxIntegrations = []Component{
			{"Build System", "queued", ""},
			{"Deployment", "queued", ""},
			{"CI/CD", "queued", ""},
			{"Documentation", "queued", ""},
		}
		qualityTesting = []Component{
			{"Testing Framework", "queued", ""},
			{"Code Quality", "queued", ""},
			{"Documentation", "queued", ""},
			{"Build Validation", "queued", ""},
		}
		userAnalyticsTesting = []Component{}
		agenticCapabilities = []Component{
			{"MAE (Auto-Migration)", "queued", ""},
			{"EngX Agent", "queued", ""},
			{"Co-Pilot", "queued", ""},
		}
		deploymentCapabilities = []Component{}
	}

	// Remove production-specific components in dev-only mode
	if isDevOnly {
		// Filter out production components based on archetype
		switch archetypeType {
		case progresssim.ArchetypeReactProd, progresssim.ArchetypeReactDev, progresssim.ArchetypeHackday:
			// Remove deployment-specific integrations for dev-only
			filteredIntegrations := make([]Component, 0)
			for _, comp := range engxIntegrations {
				if comp.Name != "GitHub Actions" {
					filteredIntegrations = append(filteredIntegrations, comp)
				}
			}
			engxIntegrations = filteredIntegrations
		}
	}

	return coreTechnologies, engxIntegrations, qualityTesting, userAnalyticsTesting, agenticCapabilities, deploymentCapabilities
}

// UpdateStep updates the current step's progress and status
func (r *EnhancedRenderer) UpdateStep(stepIndex int, progress float64, message string, subSteps []string) {
	if stepIndex >= 0 && stepIndex < len(r.steps) {
		// Don't overwrite completed steps
		if r.steps[stepIndex].Status == StepComplete {
			return
		}

		r.steps[stepIndex].Progress = progress
		r.steps[stepIndex].Message = message
		r.steps[stepIndex].SubSteps = subSteps

		if progress >= 1.0 {
			r.steps[stepIndex].Status = StepComplete
			r.steps[stepIndex].Duration = time.Since(r.startTime)
		} else if progress > 0 {
			r.steps[stepIndex].Status = StepRunning
		}

		// Update component statuses based on current step progress
		if stepIndex < len(r.steps) {
			stepName := r.steps[stepIndex].Name
			r.UpdateComponentStatuses(stepName, progress)
		}
	}
}

// SetCurrentStep sets which step is currently active
func (r *EnhancedRenderer) SetCurrentStep(stepIndex int) {
	r.currentStep = stepIndex
	if stepIndex >= 0 && stepIndex < len(r.steps) {
		r.steps[stepIndex].Status = StepRunning
	}
}

// CompleteStep marks a step as complete
func (r *EnhancedRenderer) CompleteStep(stepIndex int, duration time.Duration) {
	if stepIndex >= 0 && stepIndex < len(r.steps) {
		r.steps[stepIndex].Status = StepComplete
		r.steps[stepIndex].Progress = 1.0
		r.steps[stepIndex].Duration = duration
	}
}

// GetOverallProgress calculates overall progress (0.0 to 1.0)
func (r *EnhancedRenderer) GetOverallProgress() float64 {
	if len(r.steps) == 0 {
		return 0.0
	}

	total := 0.0
	for _, step := range r.steps {
		total += step.Progress
	}
	return total / float64(len(r.steps))
}

// Render generates the comprehensive enhanced output
func (r *EnhancedRenderer) Render(width int) string {
	var output strings.Builder

	// Store the width for consistent formatting
	r.totalWidth = width

	// Header section (includes empty line between header and progress)
	output.WriteString(r.renderHeader())
	output.WriteString("\n")

	// Current step info
	allComplete := r.GetOverallProgress() >= 1.0
	if r.currentStep >= 0 && r.currentStep < len(r.steps) {
		currentStepInfo := r.steps[r.currentStep]
		if currentStepInfo.Status == StepRunning || allComplete {
			output.WriteString(r.renderCurrentStepInfo(currentStepInfo))
			output.WriteString("\n")
		}
	}

	// Empty line before separator
	output.WriteString("\n")

	// Top separator
	output.WriteString(r.renderSeparatorLine())
	output.WriteString("\n")

	// Main steps section
	for i, step := range r.steps {
		stepLine := r.renderStepLine(i, step)
		output.WriteString(stepLine)
		output.WriteString("\n")
	}

	// Middle separator
	output.WriteString(r.renderSeparatorLine())
	output.WriteString("\n")

	// Footer info section
	output.WriteString(r.renderFooterInfo())
	output.WriteString("\n")

	// Bottom separator
	output.WriteString(r.renderSeparatorLine())
	output.WriteString("\n")

	// Empty line for breathing space (as shown in template)
	output.WriteString("\n")

	// Application Components section
	output.WriteString(r.renderApplicationComponents())

	// Final separator
	output.WriteString(r.renderSeparatorLine())

	return output.String()
}

// renderHeader creates the header with app name and total progress
func (r *EnhancedRenderer) renderHeader() string {
	// Determine setup type and colors based on devOnly flag
	var setupType, setupColor string
	if r.isDevOnly {
		setupType = "DEV SETUP"
		setupColor = colorBlue
	} else {
		setupType = "PRODUCTION READY SETUP"
		setupColor = colorBrightOrange
	}

	// Create colored header components
	dashPrefix := fmt.Sprintf("%s---%s", colorLightGrey, colorReset)
	coloredAppName := fmt.Sprintf("%s'%s'%s", colorBrightMagenta, r.appName, colorReset)
	creatingText := fmt.Sprintf(" Creating %s ", coloredAppName)

	// Calculate padding for setup type (accounting for color codes in length calculation)
	plainHeader := fmt.Sprintf("--- Creating '%s' ", r.appName)
	plainSetupPadding := fmt.Sprintf(" %s ", setupType)
	plainTotalLength := len(plainHeader) + len(plainSetupPadding) + 4 // 4 dashes at end

	var headerText string
	if plainTotalLength < r.totalWidth {
		middlePadding := r.totalWidth - len(plainHeader) - len(plainSetupPadding) - 4
		middleDashes := fmt.Sprintf("%s%s%s", colorLightGrey, strings.Repeat("-", middlePadding), colorReset)
		coloredSetupType := fmt.Sprintf(" %s%s%s ", setupColor, setupType, colorReset)
		endDashes := fmt.Sprintf("%s----%s", colorLightGrey, colorReset)

		headerText = dashPrefix + creatingText + middleDashes + coloredSetupType + endDashes
	} else {
		coloredSetupType := fmt.Sprintf(" %s%s%s ", setupColor, setupType, colorReset)
		endDashes := fmt.Sprintf("%s----%s", colorLightGrey, colorReset)
		headerText = dashPrefix + creatingText + coloredSetupType + endDashes
	}

	// Total progress line with modular progress bar system
	overallProgress := r.GetOverallProgress()

	// Determine overall progress state
	var progressState ProgressState
	if overallProgress >= 1.0 {
		progressState = StateDone
	} else if overallProgress > 0 {
		progressState = StateRunning
	} else {
		progressState = StateQueued
	}

	// Use modular progress bar system with terminal-width awareness
	// Calculate used space: "Total Progress: " + " " + "100.0%" (percentage text only)
	usedSpace := len("Total Progress: ") + 6 // 6 ensures "100.0%" is never truncated

	config := ProgressBarConfig{
		Width:          47,           // Fallback width
		ShowPercentage: true,         // Show percentage
		PercentagePad:  0,            // No padding - we'll handle alignment manually
		FillMode:       true,         // Use terminal-width aware sizing
		AvailableWidth: r.totalWidth, // Use full terminal width
		UsedWidth:      usedSpace,    // Space used by text and percentage
	}
	progressResult := r.renderModularProgressBar(overallProgress, progressState, config)

	progressText := fmt.Sprintf("Total Progress: %s", progressResult.Combined)

	return fmt.Sprintf("%s\n\n%s", headerText, progressText)
}

// renderCurrentStepInfo shows the current running step with colored spinner or completion
func (r *EnhancedRenderer) renderCurrentStepInfo(step Step) string {
	// Check if all steps are complete
	allComplete := r.GetOverallProgress() >= 1.0

	var message, statusText string

	if allComplete {
		// Show completion state with green color
		message = "Completed Successfully"
		statusText = fmt.Sprintf("%s✓ Done%s", colorGreen, colorReset)
	} else {
		// Show running state with colored spinner
		spinnerChars := []string{"⠋", "⠙", "⠹", "⠸", "⠼", "⠴", "⠦", "⠧", "⠇", "⠏"}
		spinnerIndex := int(time.Since(r.startTime)/time.Millisecond/100) % len(spinnerChars)
		spinner := spinnerChars[spinnerIndex]

		message = step.Message
		if message == "" {
			message = step.Name
		}

		// Determine spinner color based on step status
		var spinnerColor string
		switch step.Status {
		case StepPending:
			spinnerColor = colorWhite // Queued
		case StepRunning:
			spinnerColor = colorBlue // Running
		case StepComplete:
			spinnerColor = colorGreen // Done
		case StepError:
			spinnerColor = colorRed // Failed
		default:
			// Determine based on progress
			if step.Progress >= 1.0 {
				spinnerColor = colorGreen
			} else if step.Progress > 0 {
				spinnerColor = colorBlue
			} else {
				spinnerColor = colorWhite
			}
		}

		coloredSpinner := fmt.Sprintf("%s%s%s", spinnerColor, spinner, colorReset)
		statusText = fmt.Sprintf("%s Running...", coloredSpinner)
	}

	// Calculate spacing to right-align the status (use plain text lengths for calculation)
	var plainStatusText string
	if allComplete {
		plainStatusText = "✓ Done"
	} else {
		plainStatusText = "⠋ Running..."
	}

	usedSpace := len("Current Step: ") + len(plainStatusText) + 1 // +1 for space between message and status
	maxMessageLength := r.totalWidth - usedSpace

	if len(message) > maxMessageLength {
		message = message[:maxMessageLength-3] + "..."
	}

	padding := maxMessageLength - len(message)
	return fmt.Sprintf("Current Step: %s%s%s", message, strings.Repeat(" ", padding), statusText)
}

// renderStepLine creates a single aligned step line with dynamic width
func (r *EnhancedRenderer) renderStepLine(index int, step Step) string {
	// Use modular status icon system
	iconType := statusIconTypeFromStepStatus(step.Status)
	icon := r.renderStatusIcon(iconType)

	// Use modular progress state system
	progressState := progressStateFromStepStatus(step.Status, step.Progress)

	// Use modular progress bar system with fixed max width
	config := ProgressBarConfig{
		Width:          25,           // Fixed max width of 25 characters
		ShowPercentage: true,         // Show percentage
		PercentagePad:  6,            // Right-align in 6 characters
		FillMode:       false,        // Use fixed width, not fill mode
	}
	progressResult := r.renderModularProgressBar(step.Progress, progressState, config)

	// Calculate width usage for layout
	plainIcon := "[ ]" // Use plain icon for width calculation (3 chars)
	usedWidth := len(plainIcon) + 1 // icon + space
	totalProgressWidth := progressResult.PlainBarWidth + 1 + progressResult.PlainPercWidth // bar + space + percentage
	stepNameWidth := r.totalWidth - usedWidth - totalProgressWidth - 1 // -1 for space before progress

	// Ensure minimum width and bounds checking
	if stepNameWidth < 5 { // Minimum of 5 characters for step name
		stepNameWidth = 5
	}

	// Use modular step label system
	labelState := stepStatusToLabelState(step.Status)
	labelConfig := StepLabelConfig{
		BaseText:         step.Name,
		ShowEllipsis:     true, // Show "..." for in-progress steps
		MaxWidth:         stepNameWidth,
		TruncateEllipsis: true, // Show "..." when truncating for width
	}
	labelResult := r.renderModularStepLabel(labelState, labelConfig)

	// Pad styled step name to calculated width (using plain text length for padding calculation)
	paddingNeeded := stepNameWidth - labelResult.ActualWidth
	var stepNamePadded string
	if paddingNeeded > 0 {
		stepNamePadded = labelResult.StyledText + strings.Repeat(" ", paddingNeeded)
	} else {
		stepNamePadded = labelResult.StyledText
	}

	// Combine with proper spacing
	return fmt.Sprintf("%s %s %s", icon, stepNamePadded, progressResult.Combined)
}

// getLanguageColor returns the appropriate color for each programming language
func getLanguageColor(language string) string {
	switch language {
	case "JavaScript":
		return colorYellow
	case "TypeScript":
		return "\033[94m" // Bright Blue
	case "GOLANG":
		return "\033[95m" // Purple/Magenta
	case "Python":
		return "\033[38;5;208m" // Orange
	case "JAVA":
		return "\033[92m" // Bright Green
	case "RUST":
		return "\033[38;5;88m" // Bright Maroon
	case "C++":
		return "\033[34m" // Pure Blue
	case "Kotlin":
		return "\033[93m" // Bright Yellow
	case "Swift":
		return "\033[38;5;206m" // Bright Pink
	case "Obj. C":
		return colorMagenta
	default:
		return colorWhite
	}
}

// getFrameworkLanguageDisplay determines the appropriate Framework - Language display based on the archetype components
// Returns colored display string and plain text for length calculations
func (r *EnhancedRenderer) getFrameworkLanguageDisplay() (string, string) {
	// Check for React-based archetypes (prod-web, dev-web, hackday)
	for _, comp := range r.coreTechnologies {
		if comp.Name == "React Router 7" || comp.Name == "Tailwind CSS" || comp.Name == "Radix UI" {
			framework := "React Router 7"
			language := "TypeScript"
			frameworkColor := colorLightGrey
			languageColor := getLanguageColor(language)

			// Format: {Framework} - {Language} with different colors
			coloredDisplay := fmt.Sprintf("%s%s%s - %s%s%s", frameworkColor, framework, colorReset, languageColor, language, colorReset)
			plainText := fmt.Sprintf("%s - %s", framework, language)
			return coloredDisplay, plainText
		}
	}

	// Check for EngX Command Plugin archetype (engx-cmd)
	for _, comp := range r.coreTechnologies {
		if comp.Name == "Terminal UI" || comp.Name == "Interactive Forms" || comp.Name == "Command Structure" || comp.Name == "Plugin System" {
			framework := "Bubble"
			language := "GOLANG"
			frameworkColor := colorLightGrey
			languageColor := getLanguageColor(language)

			coloredDisplay := fmt.Sprintf("%s%s%s - %s%s%s", frameworkColor, framework, colorReset, languageColor, language, colorReset)
			plainText := fmt.Sprintf("%s - %s", framework, language)
			return coloredDisplay, plainText
		}
	}

	// Check for CLI Tool archetype (cli)
	for _, comp := range r.coreTechnologies {
		if comp.Name == "Argument Parsing" || comp.Name == "Command Line Interface" || comp.Name == "Help System" {
			framework := "Bubble"
			language := "GOLANG"
			frameworkColor := colorLightGrey
			languageColor := getLanguageColor(language)

			coloredDisplay := fmt.Sprintf("%s%s%s - %s%s%s", frameworkColor, framework, colorReset, languageColor, language, colorReset)
			plainText := fmt.Sprintf("%s - %s", framework, language)
			return coloredDisplay, plainText
		}
	}

	// Check for Backend Service archetype (service) - minimal components
	for _, comp := range r.coreTechnologies {
		if comp.Name == "HTTP API" || comp.Name == "Database Integration" || comp.Name == "Background Processing" {
			framework := "Bubble"
			language := "GOLANG"
			frameworkColor := colorLightGrey
			languageColor := getLanguageColor(language)

			coloredDisplay := fmt.Sprintf("%s%s%s - %s%s%s", frameworkColor, framework, colorReset, languageColor, language, colorReset)
			plainText := fmt.Sprintf("%s - %s", framework, language)
			return coloredDisplay, plainText
		}
	}

	// Fallback to template-based logic for edge cases
	switch strings.ToLower(r.template) {
	case "typescript":
		language := "TypeScript"
		languageColor := getLanguageColor(language)
		return fmt.Sprintf("%s%s%s", languageColor, language, colorReset), language
	case "javascript":
		language := "JavaScript"
		languageColor := getLanguageColor(language)
		return fmt.Sprintf("%s%s%s", languageColor, language, colorReset), language
	default:
		return r.template, r.template
	}
}

// renderFooterInfo creates the footer with timing and directory info
func (r *EnhancedRenderer) renderFooterInfo() string {
	// First line: Target Directory and Framework-Language with colors
	templateDisplay, plainTemplateText := r.getFrameworkLanguageDisplay()

	// Color the directory path bright magenta
	coloredTargetDir := fmt.Sprintf("%s%s%s", colorBrightMagenta, r.targetDir, colorReset)

	// Calculate padding accounting for the original text length (without color codes)
	plainLine1 := fmt.Sprintf("Target Directory: %s", r.targetDir)
	padding1 := r.totalWidth - len(plainLine1) - len(plainTemplateText)

	var line1 string
	if padding1 > 0 {
		line1 = fmt.Sprintf("Target Directory: %s%s%s", coloredTargetDir, strings.Repeat(" ", padding1), templateDisplay)
	} else {
		line1 = fmt.Sprintf("Target Directory: %s %s", coloredTargetDir, templateDisplay)
	}

	// Second line: Timing information
	elapsed := time.Since(r.startTime)
	elapsedFormatted := formatDuration(elapsed)

	// Estimate remaining time based on progress
	var estimatedRemaining string
	if progress := r.GetOverallProgress(); progress > 0 && progress < 1.0 {
		totalEstimated := time.Duration(float64(elapsed) / progress)
		remaining := totalEstimated - elapsed
		estimatedRemaining = formatDuration(remaining)
	} else {
		estimatedRemaining = "00h 00m 00s"
	}

	line2Left := fmt.Sprintf("Estimated Time Remaining: %s", estimatedRemaining)
	line2Right := fmt.Sprintf("Elapsed Time: %s", elapsedFormatted)
	padding2 := r.totalWidth - len(line2Left) - len(line2Right)
	if padding2 > 0 {
		line2 := line2Left + strings.Repeat(" ", padding2) + line2Right
		return fmt.Sprintf("%s\n%s", line1, line2)
	}

	return line1
}

// renderApplicationComponents creates the Application Components section
func (r *EnhancedRenderer) renderApplicationComponents() string {
	var output strings.Builder

	// Section header - white title with grey dashes
	headerText := "---- APPLICATION COMPONENTS "
	padding := r.totalWidth - len(headerText)
	var fullHeaderText string
	if padding > 0 {
		paddingDashes := fmt.Sprintf("%s%s%s", colorLightGrey, strings.Repeat("-", padding), colorReset)
		// Use grey for dashes but white for title
		dashPrefix := fmt.Sprintf("%s----%s", colorLightGrey, colorReset)
		whiteTitle := fmt.Sprintf("%s APPLICATION COMPONENTS %s", colorWhite, colorReset)
		fullHeaderText = dashPrefix + whiteTitle + paddingDashes
	} else {
		dashPrefix := fmt.Sprintf("%s----%s", colorLightGrey, colorReset)
		whiteTitle := fmt.Sprintf("%s APPLICATION COMPONENTS %s", colorWhite, colorReset)
		fullHeaderText = dashPrefix + whiteTitle
	}
	output.WriteString(fullHeaderText + "\n")
	output.WriteString("\n") // Empty line after header for breathing room

	// Core Technologies section
	output.WriteString("• Core Technologies:\n")
	for _, component := range r.coreTechnologies {
		output.WriteString(r.renderComponentLine(component))
	}
	output.WriteString("\n")

	// EngX Integrations section
	output.WriteString("• EngX Integrations:\n")
	for _, component := range r.engxIntegrations {
		output.WriteString(r.renderComponentLine(component))
	}
	output.WriteString("\n")

	// Quality & Testing section
	output.WriteString("• Quality & Testing:\n")
	for _, component := range r.qualityTesting {
		output.WriteString(r.renderComponentLine(component))
	}
	output.WriteString("\n")

	// User Analytics & Testing section
	output.WriteString("• User Analytics & Testing:\n")
	for _, component := range r.userAnalyticsTesting {
		output.WriteString(r.renderComponentLine(component))
	}
	output.WriteString("\n")

	// Agentic Capabilities section
	output.WriteString("• Agentic Capabilities:\n")
	for _, component := range r.agenticCapabilities {
		output.WriteString(r.renderComponentLine(component))
	}
	output.WriteString("\n")

	// Deployment Capabilities section
	output.WriteString("• Deployment Capabilities:\n")
	for _, component := range r.deploymentCapabilities {
		output.WriteString(r.renderComponentLine(component))
	}

	output.WriteString("\n") // Empty line before footer for breathing room

	return output.String()
}

// renderComponentLine creates a properly aligned component line
func (r *EnhancedRenderer) renderComponentLine(component Component) string {
	// Use new modular status system
	componentState := componentStateFromStatus(component.Status)
	statusDisplay := r.renderComponentInstallationStatus(componentState)

	// Generate colored icon based on status if not provided
	var icon string
	if component.Icon == "" {
		// Use modular status icon system for components
		var iconType StatusIconType
		switch componentState {
		case ComponentQueued:
			iconType = IconQueued
		case ComponentInstalling:
			iconType = IconRunning
		case ComponentInstalled:
			iconType = IconComplete
		case ComponentFailed:
			iconType = IconError
		case ComponentSkipped:
			iconType = IconPending // Use pending style for skipped
		default:
			iconType = IconQueued
		}
		icon = r.renderStatusIcon(iconType)
	} else {
		icon = component.Icon
	}

	// Calculate dynamic width for proper right alignment
	// Note: we need to calculate the plain text length for proper alignment since statusDisplay contains color codes
	var plainStatusLength int
	switch componentState {
	case ComponentQueued:
		plainStatusLength = len("[queued]")
	case ComponentInstalling:
		plainStatusLength = len("[installing...]")
	case ComponentSkipped:
		plainStatusLength = len("[skipped]")
	case ComponentInstalled:
		plainStatusLength = len("[installed]")
	case ComponentFailed:
		plainStatusLength = len("[failed]")
	default:
		plainStatusLength = len("[queued]")
	}

	// Calculate icon width for plain text (without color codes)
	plainIconLength := 3 // "[ ]", "[✓]", "[✗]" are all 3 characters

	// Total width - indent - icon - spaces - status = remaining for component name
	remainingWidth := r.totalWidth - 2 - plainIconLength - 1 - plainStatusLength - 1

	// Use modular step label system for component names
	labelState := componentStatusToLabelState(component.Status)
	labelConfig := StepLabelConfig{
		BaseText:         component.Name,
		ShowEllipsis:     true, // Show "..." for installing components
		MaxWidth:         remainingWidth,
		TruncateEllipsis: true, // Show "..." when truncating for width
	}
	labelResult := r.renderModularStepLabel(labelState, labelConfig)

	// Pad styled component name to calculated width (using plain text length for padding calculation)
	paddingNeeded := remainingWidth - labelResult.ActualWidth
	var componentNamePadded string
	if paddingNeeded > 0 {
		componentNamePadded = labelResult.StyledText + strings.Repeat(" ", paddingNeeded)
	} else {
		componentNamePadded = labelResult.StyledText
	}

	// Combine with proper spacing
	return fmt.Sprintf("  %s %s %s\n", icon, componentNamePadded, statusDisplay)
}

// ProgressState represents different states for progress visualization
type ProgressState int

const (
	StateQueued ProgressState = iota
	StateRunning
	StateStalling
	StateFailed
	StateDone
)

// renderProgressBar creates a colored progress bar with the specified width
func (r *EnhancedRenderer) renderProgressBar(progress float64, width int) string {
	return r.renderProgressBarWithState(progress, width, StateRunning)
}

// renderProgressBarWithState creates a colored progress bar with specific state
func (r *EnhancedRenderer) renderProgressBarWithState(progress float64, width int, state ProgressState) string {
	filled := int(progress * float64(width))
	empty := width - filled

	if filled < 0 {
		filled = 0
	}
	if empty < 0 {
		empty = 0
	}

	// Determine color based on state
	var barColor string
	switch state {
	case StateQueued:
		barColor = colorWhite
	case StateRunning:
		barColor = colorBlue
	case StateStalling:
		barColor = colorYellow
	case StateFailed:
		barColor = colorRed
	case StateDone:
		barColor = colorGreen
	default:
		// If progress is 100%, use green regardless of state
		if progress >= 1.0 {
			barColor = colorGreen
		} else if progress > 0 {
			barColor = colorBlue
		} else {
			barColor = colorWhite
		}
	}

	// Create colored progress bar
	coloredFilled := fmt.Sprintf("%s%s%s", barColor, strings.Repeat("#", filled), colorReset)
	emptySpace := strings.Repeat(" ", empty)

	return fmt.Sprintf("[%s%s]", coloredFilled, emptySpace)
}

// renderColoredPercentage colors percentage text to match progress bar state
func (r *EnhancedRenderer) renderColoredPercentage(progress float64, state ProgressState) string {
	// Calculate percentage with proper rounding
	percentage := progress * 100
	var percentText string
	if percentage <= 0.0 {
		percentText = "0.0%"
	} else {
		// Format to 1 decimal place first
		percentText = fmt.Sprintf("%.1f%%", percentage)
		// Force correction for any "100%" to "100.0%"
		if percentText == "100%" {
			percentText = "100.0%"
		}
	}

	// Determine color based on state (same logic as progress bars)
	var percentColor string
	switch state {
	case StateQueued:
		percentColor = colorWhite
	case StateRunning:
		percentColor = colorBlue
	case StateStalling:
		percentColor = colorYellow
	case StateFailed:
		percentColor = colorRed
	case StateDone:
		percentColor = colorGreen
	default:
		if progress >= 1.0 {
			percentColor = colorGreen
		} else if progress > 0 {
			percentColor = colorBlue
		} else {
			percentColor = colorWhite
		}
	}

	return fmt.Sprintf("%s%s%s", percentColor, percentText, colorReset)
}

// StatusIconType represents different types of status indicators
type StatusIconType int

const (
	IconPending StatusIconType = iota
	IconRunning
	IconComplete
	IconError
	IconQueued  // Alias for pending
)

// renderStatusIcon creates a colored status icon based on type
func (r *EnhancedRenderer) renderStatusIcon(iconType StatusIconType) string {
	var icon, color string

	switch iconType {
	case IconPending, IconQueued:
		icon = "[ ]"
		color = colorWhite
	case IconRunning:
		icon = "[✓ ]" // Space after checkmark for running steps
		color = colorBlue
	case IconComplete:
		icon = "[✓]"
		color = colorGreen
	case IconError:
		icon = "[✗]"
		color = colorRed
	default:
		icon = "[ ]"
		color = colorWhite
	}

	return fmt.Sprintf("%s%s%s", color, icon, colorReset)
}

// statusIconTypeFromStepStatus converts step status to icon type
func statusIconTypeFromStepStatus(status StepStatus) StatusIconType {
	switch status {
	case StepPending:
		return IconPending
	case StepRunning:
		return IconRunning
	case StepComplete:
		return IconComplete
	case StepError:
		return IconError
	default:
		return IconPending
	}
}

// ProgressBarConfig configures how a progress bar should be rendered
type ProgressBarConfig struct {
	Width           int    // Fixed width in characters, or 0 for "fill" mode
	ShowPercentage  bool   // Whether to show percentage readout
	PercentagePad   int    // Padding for percentage alignment (e.g., 6 for " 100.0%")
	FillMode        bool   // If true, calculate width to fill remaining space
	AvailableWidth  int    // Total available width for fill mode calculations
	UsedWidth       int    // Already used width (for fill mode calculations)
}

// ProgressBarResult contains the rendered progress bar and related info
type ProgressBarResult struct {
	ProgressBar     string // The rendered progress bar [####    ]
	Percentage      string // The colored percentage text
	Combined        string // Bar + percentage combined with proper spacing
	PlainBarWidth   int    // Actual width of the progress bar (without colors)
	PlainPercWidth  int    // Actual width of percentage text (without colors)
}

// StepLabelState represents different states for step/component labels
type StepLabelState int

const (
	LabelQueued StepLabelState = iota
	LabelInProgress
	LabelPaused
	LabelFailed
	LabelSuccess
	LabelSkipped
)

// StepLabelConfig configures how a step/component label should be rendered
type StepLabelConfig struct {
	BaseText           string // The base text to display
	ShowEllipsis       bool   // Whether to show "..." for in-progress state
	MaxWidth           int    // Maximum width before truncation (0 = no limit)
	TruncateEllipsis   bool   // Whether to show "..." when truncating for width
}

// StepLabelResult contains the rendered step label and metadata
type StepLabelResult struct {
	StyledText    string // The fully styled text with colors and formatting
	PlainText     string // The plain text without styling
	ActualWidth   int    // The actual character width of the plain text
}

// renderModularStepLabel creates a styled step/component label based on state
func (r *EnhancedRenderer) renderModularStepLabel(state StepLabelState, config StepLabelConfig) StepLabelResult {
	var color, style, suffix string

	// Determine styling based on state
	switch state {
	case LabelQueued:
		color = colorWhite
		style = ""
		suffix = ""
	case LabelInProgress:
		color = colorBlue
		style = styleItalic
		suffix = ""
		if config.ShowEllipsis {
			suffix = "..."
		}
	case LabelPaused:
		color = colorYellow
		style = ""
		suffix = ""
	case LabelFailed:
		color = colorRed
		style = ""
		suffix = ""
	case LabelSuccess:
		color = colorGreen
		style = ""
		suffix = ""
	case LabelSkipped:
		color = colorLightGrey
		style = styleItalic
		suffix = ""
	default:
		color = colorWhite
		style = ""
		suffix = ""
	}

	// Prepare the text with suffix
	displayText := config.BaseText + suffix

	// Handle width truncation if specified
	if config.MaxWidth > 0 && len(displayText) > config.MaxWidth {
		if config.TruncateEllipsis && config.MaxWidth > 3 {
			displayText = displayText[:config.MaxWidth-3] + "..."
		} else {
			displayText = displayText[:config.MaxWidth]
		}
	}

	// Apply styling
	var styledText string
	if style != "" {
		styledText = fmt.Sprintf("%s%s%s%s%s", color, style, displayText, styleReset, colorReset)
	} else {
		styledText = fmt.Sprintf("%s%s%s", color, displayText, colorReset)
	}

	return StepLabelResult{
		StyledText:  styledText,
		PlainText:   displayText,
		ActualWidth: len(displayText),
	}
}

// stepStatusToLabelState converts StepStatus to StepLabelState
func stepStatusToLabelState(status StepStatus) StepLabelState {
	switch status {
	case StepPending:
		return LabelQueued
	case StepRunning:
		return LabelInProgress
	case StepComplete:
		return LabelSuccess
	case StepError:
		return LabelFailed
	default:
		return LabelQueued
	}
}

// componentStatusToLabelState converts component status string to StepLabelState
func componentStatusToLabelState(status string) StepLabelState {
	switch status {
	case "queued":
		return LabelQueued
	case "installing":
		return LabelInProgress
	case "installed":
		return LabelSuccess
	case "failed":
		return LabelFailed
	case "skipped":
		return LabelSkipped
	default:
		return LabelQueued
	}
}

// renderModularProgressBar creates a configurable progress bar with optional percentage
func (r *EnhancedRenderer) renderModularProgressBar(progress float64, state ProgressState, config ProgressBarConfig) ProgressBarResult {
	// Calculate actual bar width
	var barWidth int
	if config.FillMode && config.AvailableWidth > 0 {
		// Calculate remaining space for bar
		usedSpace := config.UsedWidth
		if config.ShowPercentage {
			if config.PercentagePad > 0 {
				usedSpace += config.PercentagePad + 1 // +1 for space between bar and percentage
			} else {
				// When PercentagePad is 0, we need to account for actual percentage width
				// Ensures "100.0%" is never truncated across different terminal widths
				usedSpace += 6 // Conservative spacing to prevent truncation
			}
		}
		barWidth = config.AvailableWidth - usedSpace
		if barWidth < 10 { // Minimum bar width
			barWidth = 10
		}
	} else {
		barWidth = config.Width
		if barWidth <= 0 {
			barWidth = 30 // Default width
		}
	}

	// Render the progress bar
	progressBar := r.renderProgressBarWithState(progress, barWidth, state)

	// Render percentage if requested
	var percentage string
	var combined string
	var plainPercWidth int

	if config.ShowPercentage {
		percentage = r.renderColoredPercentage(progress, state)
		// Calculate plain text width for percentage
		plainPercText := fmt.Sprintf("%.1f%%", progress*100)
		if plainPercText == "100%" {
			plainPercText = "100.0%"
		}
		plainPercWidth = len(plainPercText)

		// Combine with proper padding
		if config.PercentagePad > 0 {
			paddedPercentage := fmt.Sprintf("%*s", config.PercentagePad, percentage)
			combined = fmt.Sprintf("%s %s", progressBar, paddedPercentage)
		} else {
			// For terminal-width aware progress bars, right-align the percentage
			combined = fmt.Sprintf("%s %s", progressBar, percentage)
		}
	} else {
		percentage = ""
		combined = progressBar
		plainPercWidth = 0
	}

	return ProgressBarResult{
		ProgressBar:    progressBar,
		Percentage:     percentage,
		Combined:       combined,
		PlainBarWidth:  barWidth + 2, // +2 for [ ]
		PlainPercWidth: plainPercWidth,
	}
}

// renderComponentInstallationStatus renders component installation status with colors
// States: [queued] White, [installing...] Blue, [skipped] Grey Italic, [installed] Green, [failed] Red
func (r *EnhancedRenderer) renderComponentInstallationStatus(state ComponentInstallationState) string {
	var status, color, style string

	switch state {
	case ComponentQueued:
		status = "[queued]"
		color = colorWhite
		style = ""
	case ComponentInstalling:
		status = "[installing...]"
		color = colorBlue
		style = ""
	case ComponentSkipped:
		status = "[skipped]"
		color = colorLightGrey
		style = styleItalic
	case ComponentInstalled:
		status = "[installed]"
		color = colorGreen
		style = ""
	case ComponentFailed:
		status = "[failed]"
		color = colorRed
		style = ""
	default:
		status = "[queued]"
		color = colorWhite
		style = ""
	}

	// Apply both color and style if present
	if style != "" {
		return fmt.Sprintf("%s%s%s%s%s", color, style, status, styleReset, colorReset)
	}
	return fmt.Sprintf("%s%s%s", color, status, colorReset)
}

// componentStateFromStatus converts string status to ComponentInstallationState
func componentStateFromStatus(status string) ComponentInstallationState {
	switch status {
	case "installed":
		return ComponentInstalled
	case "installing":
		return ComponentInstalling
	case "skipped":
		return ComponentSkipped
	case "failed":
		return ComponentFailed
	case "queued":
		return ComponentQueued
	default:
		return ComponentQueued
	}
}

// progressStateFromStepStatus converts step status to progress state
func progressStateFromStepStatus(status StepStatus, progress float64) ProgressState {
	switch status {
	case StepPending:
		return StateQueued
	case StepRunning:
		return StateRunning
	case StepComplete:
		return StateDone
	case StepError:
		return StateFailed
	default:
		// Fallback based on progress value
		if progress >= 1.0 {
			return StateDone
		} else if progress > 0 {
			return StateRunning
		} else {
			return StateQueued
		}
	}
}

// formatDuration formats a duration into HH:MM:SS format
func formatDuration(d time.Duration) string {
	hours := int(d.Hours())
	minutes := int(d.Minutes()) % 60
	seconds := int(d.Seconds()) % 60
	return fmt.Sprintf("%02dh %02dm %02ds", hours, minutes, seconds)
}

// GetStepCount returns the number of steps
func (r *EnhancedRenderer) GetStepCount() int {
	return len(r.steps)
}

// GetStepAtIndex returns the step at the given index for debugging
func (r *EnhancedRenderer) GetStepAtIndex(index int) *Step {
	if index >= 0 && index < len(r.steps) {
		return &r.steps[index]
	}
	return nil
}

// UpdateComponentStatuses updates component statuses based on step progress using the modular system
func (r *EnhancedRenderer) UpdateComponentStatuses(stepName string, stepProgress float64) {
	// Map step name to installation phase
	phase := MapStepNameToPhase(stepName)

	// Get all component updates up to this phase and progress
	// This ensures previous phase components remain installed and don't re-animate
	updates := r.componentManager.GetAllComponentsUpToPhase(phase, stepProgress)

	// Apply updates to components (with protection against re-animation)
	for _, update := range updates {
		r.applyComponentUpdateSafe(update)
	}

	// Special case: Finalizing step - ensure everything is installed
	if (stepName == "Finalizing Setup" || stepName == "Finalizing setup") && stepProgress >= 1.0 {
		allUpdates := r.componentManager.GetAllComponentsForPhase(PhaseFinalizing)
		for _, update := range allUpdates {
			r.applyComponentUpdateSafe(update)
		}
	}
}

// applyComponentUpdate applies a component update to the appropriate section
func (r *EnhancedRenderer) applyComponentUpdate(update ComponentUpdate) {
	// Try to update in core technologies
	for i := range r.coreTechnologies {
		if r.coreTechnologies[i].Name == update.ComponentName {
			r.coreTechnologies[i].Status = update.NewStatus
			// Icon is generated by modular system, no need to set it
			return
		}
	}

	// Try to update in engx integrations
	for i := range r.engxIntegrations {
		if r.engxIntegrations[i].Name == update.ComponentName {
			r.engxIntegrations[i].Status = update.NewStatus
			// Icon is generated by modular system, no need to set it
			return
		}
	}

	// Try to update in all 6 component sections
	for i := range r.qualityTesting {
		if r.qualityTesting[i].Name == update.ComponentName {
			r.qualityTesting[i].Status = update.NewStatus
			return
		}
	}

	for i := range r.engxIntegrations {
		if r.engxIntegrations[i].Name == update.ComponentName {
			r.engxIntegrations[i].Status = update.NewStatus
			return
		}
	}

	for i := range r.userAnalyticsTesting {
		if r.userAnalyticsTesting[i].Name == update.ComponentName {
			r.userAnalyticsTesting[i].Status = update.NewStatus
			return
		}
	}

	for i := range r.agenticCapabilities {
		if r.agenticCapabilities[i].Name == update.ComponentName {
			r.agenticCapabilities[i].Status = update.NewStatus
			return
		}
	}

	for i := range r.deploymentCapabilities {
		if r.deploymentCapabilities[i].Name == update.ComponentName {
			r.deploymentCapabilities[i].Status = update.NewStatus
			return
		}
	}
}

// applyComponentUpdateSafe applies a component update but prevents downgrading from installed to installing
func (r *EnhancedRenderer) applyComponentUpdateSafe(update ComponentUpdate) {
	// Try to update in core technologies
	for i := range r.coreTechnologies {
		if r.coreTechnologies[i].Name == update.ComponentName {
			// Don't downgrade from installed to installing
			if r.coreTechnologies[i].Status == "installed" && update.NewStatus == "installing" {
				return
			}
			r.coreTechnologies[i].Status = update.NewStatus
			// Icon is generated by modular system, no need to set it
			return
		}
	}

	// Try to update in engx integrations
	for i := range r.engxIntegrations {
		if r.engxIntegrations[i].Name == update.ComponentName {
			// Don't downgrade from installed to installing
			if r.engxIntegrations[i].Status == "installed" && update.NewStatus == "installing" {
				return
			}
			r.engxIntegrations[i].Status = update.NewStatus
			// Icon is generated by modular system, no need to set it
			return
		}
	}

	// Try to update in all 6 component sections with safe downgrade protection
	for i := range r.qualityTesting {
		if r.qualityTesting[i].Name == update.ComponentName {
			// Don't downgrade from installed to installing
			if r.qualityTesting[i].Status == "installed" && update.NewStatus == "installing" {
				return
			}
			r.qualityTesting[i].Status = update.NewStatus
			return
		}
	}

	for i := range r.engxIntegrations {
		if r.engxIntegrations[i].Name == update.ComponentName {
			if r.engxIntegrations[i].Status == "installed" && update.NewStatus == "installing" {
				return
			}
			r.engxIntegrations[i].Status = update.NewStatus
			return
		}
	}

	for i := range r.userAnalyticsTesting {
		if r.userAnalyticsTesting[i].Name == update.ComponentName {
			if r.userAnalyticsTesting[i].Status == "installed" && update.NewStatus == "installing" {
				return
			}
			r.userAnalyticsTesting[i].Status = update.NewStatus
			return
		}
	}

	for i := range r.agenticCapabilities {
		if r.agenticCapabilities[i].Name == update.ComponentName {
			if r.agenticCapabilities[i].Status == "installed" && update.NewStatus == "installing" {
				return
			}
			r.agenticCapabilities[i].Status = update.NewStatus
			return
		}
	}

	for i := range r.deploymentCapabilities {
		if r.deploymentCapabilities[i].Name == update.ComponentName {
			if r.deploymentCapabilities[i].Status == "installed" && update.NewStatus == "installing" {
				return
			}
			r.deploymentCapabilities[i].Status = update.NewStatus
			return
		}
	}
}

// updateComponentStatus updates a specific component's status
func (r *EnhancedRenderer) updateComponentStatus(section, name, status, icon string) {
	if section == "coreTech" {
		for i := range r.coreTechnologies {
			if r.coreTechnologies[i].Name == name {
				r.coreTechnologies[i].Status = status
				// Icon is generated by modular system, no need to set it
				break
			}
		}
	} else if section == "engx" {
		for i := range r.engxIntegrations {
			if r.engxIntegrations[i].Name == name {
				r.engxIntegrations[i].Status = status
				// Icon is generated by modular system, no need to set it
				break
			}
		}
	}
}

// updateQualityComponentStatus updates a quality component's status
func (r *EnhancedRenderer) updateQualityComponentStatus(name, status string) {
	for i := range r.qualityTesting {
		if r.qualityTesting[i].Name == name {
			r.qualityTesting[i].Status = status
			break
		}
	}
}