...
func (p *Plugin) Description() string {
	return "Test error message formatting (development tool)"
}
...
func (p *Plugin) Create(deps interface{}) *cobra.Command {
	dependencies, ok := deps.(*common.Dependencies)
	if !ok {
		panic("invalid dependencies type")
	}
	p.deps = dependencies
...
Examples:
  ...
  engx test-error --type=resource_exhausted --chaos --severity=fatal`,
		RunE: func(cmd *cobra.Command, args []string) error {
			return p.executeCommand(cmd, args, dependencies, errorType, severity, chaosMode)
		},
	}
...
func (p *Plugin) executeCommand(cmd *cobra.Command, args []string, deps *common.Dependencies, errorType, severity string, chaosMode bool) error {
	// Parse severity
...
// Plugin export - required for plugin loading
func NewPlugin() interfaces.CommandPlugin {
	return &Plugin{}
}